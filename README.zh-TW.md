...
| Tool | Status | Notes |
| --- | --- | --- |
| OpenCode | Tested | 使用 `.opencode/plugins/yet-another-boss-request.js`，對新建立的空 session 自動啟動 YABR |
...
| OpenCode | `opencode.json`、`.opencode/plugins/`、`.agents/skills/` | 執行 `opencode .` | 載入 project skills，並在新建立空 session 的 `session.created` event 啟動 YABR |

Claude Code 與 Codex 的 hook 主要負責注入 context；OpenCode 額外提供 autostart，會從新建立空 session 的 `session.created` event 啟動，且不會接管 `opencode -s` 指定的既有 session。若 Desktop client 的 New Session 只建立前端 draft，plugin 會 fallback 到該 session 第一則 user message 前面加上 YABR startup prompt。Codex hook 目前使用 `git rev-parse --show-toplevel` 尋找專案根目錄，因此需要在 Git repo 內執行。
...
OpenCode adapter 會在新建立的空 session 觸發 `session.created` event 時啟動：
...
請使用你自己的 OpenCode Desktop model picker 或 `/api/model` 中可用的 model ID。這個檔案是本機 override，不應提交到 Git。
...
目前測試版本：OpenCode TUI `1.14.51`。OpenCode Desktop prompt autostart 需要 Desktop `>= 1.15.10`；較舊的 Desktop 版本仍會使用第一則訊息 fallback。
