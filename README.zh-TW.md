...
| OpenCode | `opencode.json`、`.opencode/plugins/`、`.agents/skills/` | 執行 `opencode .` | 載入 project skills，並對新建立的空 session 自動送出 `start YABR` 啟動 prompt |

Claude Code 與 Codex 的 hook 主要負責注入 context；OpenCode 額外提供 autostart，會對新建立的空 session 送出啟動 prompt，且不會接管 `opencode -s` 指定的既有 session。Codex hook 目前使用 `git rev-parse --show-toplevel` 尋找專案根目錄，因此需要在 Git repo 內執行。
...
OpenCode adapter 會在新建立的 session 為空時自動送出：
